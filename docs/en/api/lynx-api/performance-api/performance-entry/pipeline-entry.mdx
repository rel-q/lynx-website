...
1. **Lynx Pipeline marked with [`__lynx_timing_flag`](/guide/performance/monitor-performance/timing-flag)**, which generate performance data as `PipelineEntry`.
...
<APITable query="lynx-api/performance-api/performance-entry/pipeline-entry" />
