...
Lynx 的核心目标在于使开发者能够运用其熟悉的 Web 技术栈，构建出具备原生级体验的应用程序。通过与 HarmonyOS 实现深度融合，我们已成功将 Lynx 的高性能渲染引擎与 HarmonyOS 的原生 UI 能力结合到一起，为开发者带来以下核心优势：
...
- **多端代码复用**：一套代码，多端运行。开发者可以使用同一份代码库，同时构建 Web、iOS、Android 和 HarmonyOS 应用，提高开发效率和代码可维护性。
- **客户端 API 访问**：通过我们的原生模块扩展能力，开发者可以在 JavaScript 中调用鸿蒙平台的原生 API，无论是系统级的服务还是自定义的 Native 模块。更多可了解[原生模块的使用指南](/zh/guide/use-native-modules.html#platform=harmony)。
...
为了在鸿蒙上实现最佳性能，我们的方案核心在于将 Lynx 的内核与鸿蒙的 ArkUI 渲染体系直接对接。
...
我们把 Lynx 的 UI 树直接映射为鸿蒙的原生组件。开发者既能利用 Lynx 的性能优势，同时也能结合 ArkTS 的丰富生态与能力。
...
- **[LynxView](/zh/api/lynx-native-api/lynx-view/load-template.html#harmony)**：它是一个 ArkTS 自定义组件，遵循 ArkTS 的生命周期并且自动处理了所有关于模板加载、内存管理和组件复用的等逻辑。开发者只需传入需要的展示 bundle 路径就能自动完成渲染。
...
我们构建了一套完整的事件处理机制，能在复杂交互场景下捕获和响应手势与事件。

- **自定义 HitTest**：在 Lynx 的 UI 树上准确查找响应事件的节点。
- **手势识别**：集成了鸿蒙系统的手势识别能力，如 `tap`、`longpress` 等。
...
若您希望以最便捷的方式开启开发，可以直接使用预构建的 Lynx Explorer。您可以根据按照下面的步骤操作安装 hap 包，轻松迈出开发的第一步。
...
如果您想对 Lynx 鸿蒙的源码有更深入的理解，也可以尝试本地构建，可以参考我们的[为 Harmony 构建 Lynx Explorer](https://github.com/lynx-family/lynx/tree/develop/explorer/harmony)指南。

### 集成 Lynx 到您的工程中

若您已经有自己的鸿蒙工程，希望将 Lynx 集成其中，我们也提供了[集成参考](/zh/guide/start/integrate-with-existing-apps.html#platform=harmony)。按照参考中的步骤，您可以将 Lynx 集成到现有的工程里，充分发挥其优势。
