...
[**HarmonyOS NEXT**](https://consumer.huawei.com/cn/harmonyos-next/) 的发布推动 HarmonyOS 纯净生态加速成型。据预测 2025 年底 HarmonyOS 安装设备将超 3000 万。用户规模快速增长下，“规模化、高效地” 的跨端方案，已成为丰富 HarmonyOS 应用生态的关键需求。

开源方案 Lynx 恰好匹配这一需求。如[ Lynx 开源博客](/blog/lynx-unlock-native-for-more.html)所介绍，Lynx 是一套帮助 Web 开发者复用现有经验，通过一份代码同时构建移动端原生界面与 Web 端界面的技术方案；此前已在多端（Android、iOS、Web）开发中验证了效率优势。若 Lynx 尽快适配 HarmonyOS 并开源，将实现双向增益 —— 对 Lynx 开发者来说，可复用已有业务逻辑、高效管理多端代码，无缝迁移至 HarmonyOS；对 HarmonyOS 生态来说，借助跨端技术方案，降低开发门槛，助力应用生态快速扩容。

过去一年多，Lynx 与 OpenHarmony 合作攻克多项行业技术难题。今天，我们激动地宣布：**Lynx 已经完成对 HarmonyOS 的深度适配，在 CSS、组件、API 等维度完整对齐其他平台，现已开源**。Lynx 为开发者提供了一套完整、高效的跨端开发解决方案,并充分利用 HarmonyOS 带来的独特优势，让 HarmonyOS 的原生体验触达更多用户。

## Lynx on HarmonyOS 打动开发者的主要优势
...
将 Lynx [platform-ui-tree](https://lynxjs.org/guide/spec.html#platform-ui-tree) 直接映射为 HarmonyOS 的原生组件。开发者既能利用 Lynx 的性能优势，同时也能结合 ArkTS 的丰富生态与能力。
...
1. **[LynxView](/api/lynx-native-api/lynx-view/load-template.html#harmony)**：ArkTS 自定义组件，遵循 ArkTS 的生命周期并且自动处理了所有关于模板加载、内存管理和组件复用的等逻辑，开发者只需传入需要的展示 bundle 路径就能自动完成渲染。
...
在 Lynx 官网中已经准备好了详细的[开发文档](/guide/start/integrate-with-existing-apps.html#platform=harmony)和[示例代码](https://github.com/lynx-family/integrating-lynx-demo-projects/tree/main/harmony/HarmonyEmptyProject)，帮助开发者快速上手。

### 使用预构建的 Lynx Explorer

若希望以最便捷的方式开启开发，可以根据按照下面的步骤操作安装 Lynx Explorer hap 包以及上手简单 Lynx 开发。
...
在 "二维码" 选项点击 “复制链接” 按钮；并粘贴到 Lynx Explorer 的 Card URL 输入框，然后点击 "Go"。
...
若开发者想对 Lynx on HarmonyOS 的源码有更多的探索，可以尝试本地构建，参考[为 Harmony 构建 Lynx Explorer](https://github.com/lynx-family/lynx/tree/develop/explorer/harmony)。

### 集成 Lynx 到已有的 APP 工程中

若开发者已经有自己的 HarmonyOS 应用工程，并希望将 Lynx 集成其中，官网也提供了[集成参考](/guide/start/integrate-with-existing-apps.html#platform=harmony)。
