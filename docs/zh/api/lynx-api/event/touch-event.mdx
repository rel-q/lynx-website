...
表示手指在触摸平面上单击，[`target`](./event#target) 是包含触摸点且距离用户最近的 `element`，仅第一根手指可以触发。当手指移动超过一定阈值时，或该 `element` 的[事件响应链](../../../guide/interaction/event-handling/event-propagation.mdx#事件响应链)上有 `element` 发生了滑动，`tap` 事件无法触发。

此外，该事件和 [`longpress`](#longpress) 事件在事件监听上互斥，即前端同时监听两个事件，则两个事件不会同时触发，[`longpress`](#longpress) 优先。

### longpress

表示手指在触摸平面上长按，[`target`](./event#target) 是包含触摸点且距离用户最近的 `element`，仅第一根手指可以触发。当手指移动超过一定阈值时，或长按间隔不足 `500 ms`，`longpress` 事件无法触发。

### click

表示手指在触摸平面上单击，[`target`](./event#target) 是包含触摸点且距离用户最近且监听了 `click` 事件的 `element`，仅第一根手指可以触发。与 [`tap`](#tap) 事件不同的是，当手指移出距离用户最近且监听了 `click` 事件的 `element` 时，或该 `element` 的[事件响应链](../../../guide/interaction/event-handling/event-propagation.mdx#事件响应链)上有 `element` 发生了滑动，`click` 事件才无法触发。
