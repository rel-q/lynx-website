...
### 设置启动阶段录制 Trace 的时间（单位：秒）

输入你期望的启动阶段录制 Trace 的时间，输入 `Enter` 键或者点击 `Confirm` 按钮保存配置。
...
应用启动完成后，选择需要分析的应用，然后点击 `Load Trace` 按钮加载启动阶段录制的 Trace 数据。
...
##### 点击 `Load Trace` 按钮后提示：Startup tracing is not enabled, cannot pull trace data

- 请确保在启动阶段 Trace 配置中设置了有效的时长（>0），然后在重启 App 后点击 `Load Trace`。
- 请确保在 App 启动过程中或启动打开 Lynx 页面后再点击 `Load Trace` 按钮。

##### 点击 `Load Trace` 按钮后提示：Startup tracing is in progress, please try again later

- App 启动阶段的 Trace 仍在录制中，请等待录制完成后再点击 `Load Trace` 按钮。
